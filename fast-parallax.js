(function () {
  ...
  window.fastParallax = {
    ...
    // Stored properties. These do not change with every scroll.
    windowHeight: null,
    windowMiddle: null,
    windowWidth: null,
    ...
    initialise: function() {
      ...
      // (-webkit-transform). This is to add support for older Webkits (Safari
      // and Android browser) that are still widely used.
      ...
      this.elements = [].slice.call(document.getElementsByClassName("fast-parallax"))

      // Bind events.
      window.addEventListener("resize", this.recalculate.bind(this));
      window.addEventListener("scroll", this.scrollHook.bind(this));
      console.log(this)
      this.recalculate()
    ...
    scrollHook: function() {
      ...
      var scrollBottom = this.windowHeight + currentPos
      ...
      for (i = 0; i < this.items.length; i++) {
        ...
        // screen?
        middleOffset = item.middle - currentPos - this.windowMiddle;
        ...
        item.element.style[this.transformProperty] = "translate3d(" + xOffset.toString() + item.unit + ", " + yOffset.toString() + item.unit + ", 0)";
      }
    }
  }

  window.addEventListener('DOMContentLoaded', window.fastParallax.initialise.bind(window.fastParallax));
})();
